        ...
        for doc in docs:
            ...
            def flush_buffer():
                ...
                if self.include_metadata:
                    ...
                    if pages:
                        metadata["page_idx"] = pages
                # Use the headers that were active when this buffer was built
                metadata["headers"] = (buffer_headers or header_stack).copy()
                nodes.append(TextNode(text=combined_text, metadata=metadata))
                            ...
                            if self.include_metadata:
                                for k, v in element.items():
                                    if k not in {"type", "text"}:
                                        metadata[k] = v
                            metadata["headers"] = header_stack.copy()
                ...
                if etype in ("table", "image"):
                    ...
                    if etype == "table":
                        ...
                        if not text_value:
                            ...
                            continue
                    nodes.append(TextNode(text=text_value, metadata=metadata))
                    continue
