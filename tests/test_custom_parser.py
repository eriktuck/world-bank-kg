...
def test_header_splits() -> None:
    ...
    doc = Document(
        text = """
            [{"type": "text", "text": "Header 1", "text_level": 1},
            {"type": "text", "text": "Paragraph under header 1."},
            {"type": "text", "text": "Header 2", "text_level": 1},
            {"type": "text", "text": "Paragraph under header 2."}]  
            """
    )
    splits = parser.get_nodes_from_documents([doc])

    assert len(splits) == 2
    assert "Header 1" in splits[0].metadata.get("header_path", "")
    assert "Paragraph under header 1." in splits[0].text
    assert "Header 2" in splits[1].metadata.get("header_path", "")
    assert "Paragraph under header 2." in splits[1].text
...
def test_pre_header_content() -> None:
    ...
    doc = Document(
        text = """
            [{"type": "text", "text": "pre-header content."},
            {"type": "text", "text": "Header 1", "text_level": 1},
            {"type": "text", "text": "Paragraph under header 1."}]  
            """
    )
    ...
    assert len(splits) == 2
    assert "pre-header content." in splits[0].text
    assert "Header 1" in splits[1].metadata.get("header_path", "")
    assert "Paragraph under header 1." in splits[1].text
...
def test_header_metadata() -> None:
    ...
    doc = Document(
        text = """
            [{"type": "text", "text": "Main Header", "text_level": 1},
            {"type": "text", "text": "Paragraph under header 1."},
            {"type": "text", "text": "Sub-header", "text_level": 2},
            {"type": "text", "text": "Paragraph under header 2."},
            {"type": "text", "text": "Sub-sub-header", "text_level": 3},
            {"type": "text", "text": "Paragraph under header 3."},
            {"type": "text", "text": "New title", "text_level": 1}]  
            """
    )
    splits = parser.get_nodes_from_documents([doc])

    assert len(splits) == 4
    assert splits[0].metadata == {"header_path": "/Main Header/"}
    assert splits[1].metadata == {"header_path": "/Main Header/Sub-header/"}
    assert splits[3].metadata == {"header_path": "/New title/"}
...
def test_table_parsing():
    ...
    doc = Document(
        text = """
            [{"type": "text", "text": "Header 1", "text_level": 1},
            {"type": "table", "table_body": "<table><tr><td>Cell 1</td><td>Cell 2</td></tr></table>"},
            {"type": "text", "text": "Paragraph after table."}]  
            """
    )
    splits = parser.get_nodes_from_documents([doc])

    assert len(splits) == 2
    assert "Cell 1" in splits[0].text
    assert "Cell 2" in splits[0].text

def test_structured_output(sample_document):
